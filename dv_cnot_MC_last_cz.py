...
def runner(i_rep):
    ...
    for i_p, p_c in enumerate(pc_list):
        ...
        for i_iter in range(Nrep):
            ...
            for i_l in range(14):
                ...
                if (i_l < 5) or (i_l >= 8 and i_l< 13): # lower/upper rung
                    cz_err_gen(err_cz,i_l,i_l+1,vec_x,vec_z)
                elif i_l == 6: # 3-7
                    cz37_err_gen(err_cz,vec_x,vec_z)
                elif i_l == 7: # 7-11
                    cz_err_gen(err_cz,7,11,vec_x,vec_z)
                elif i_l == 5 or i_l == 13:
                    id_or_err = (np.random.rand() < p_c)
                    if id_or_err:
                        pauli = 1+np.random.randint(15) # I X Z Y =  0, 1, 2, 3
                        q1 = pauli%4
                        vec_x[i_l] += (q1%2) 
                        vec_z[i_l] += int(q1/2) 
                        q2 = int(pauli/4)
                        vec_x[i_l+1] += (q2%2) 
                        vec_z[i_l+1] += int(q2/2)                         
    ...
    fname = "data_dv_cnot/" + "last_cz_pm_0_i_%d.npz" % (i_rep)
    # fname = "data_dv_cnot/" + "last_cz_pc_0_i_%d.npz" % (i_rep)
    # fname = "data_dv_cnot/" + "last_cz_pm_eq_pc_i_%d.npz" % (i_rep)
