...
def default_noise_model():
    ...
    #depol_one_qb_error = 0.00
    #depol_two_qb_error = 0.0
    ...
    one_qb_error = p_meas*2/3*4
    noise.add_all_qubit_quantum_error(depolarizing_error(p_meas*(3+1*k)*2/3,1), ['u1'])
    noise.add_all_qubit_quantum_error(depolarizing_error(p_meas*(2+2*k)*2/3,1), ['u2'])        
    noise.add_all_qubit_quantum_error(depolarizing_error(p_meas*(1+3*k)*2/3,1), ['u3'])
    noise.add_all_qubit_quantum_error(depolarizing_error(one_qb_error,1), ['h', 's', 'sdg'])

    two_qb_error = p_meas*2/3*15*5
    ...
    reset_to_zero_error = 0.00
    reset_to_one_error = 0.00
...
def set_noise_model(noise_model = None):
    ...
    ```
        ...
        one_qb_error = p_meas*2/3*4
        noise.add_all_qubit_quantum_error(amplitude_damping_error(p_meas*(3+1*k))*2/3, ['u1'])
        noise.add_all_qubit_quantum_error(amplitude_damping_error(p_meas*(2+2*k))*2/3, ['u2'])        
        noise.add_all_qubit_quantum_error(amplitude_damping_error(p_meas*(1+3*k))*2/3, ['u3'])
	noise.add_all_qubit_quantum_error(amplitude_damping_error(one_qb_error), ['h', 's', 'sdg'])
